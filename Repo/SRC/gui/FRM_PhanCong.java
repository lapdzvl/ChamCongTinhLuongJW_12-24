	...
	public FRM_PhanCong() {
		...
		tblDSTLD.addMouseListener(new MouseAdapter() {
			...
			public void mouseClicked(MouseEvent e) {
                ...
                //lap add
                tblDSPC.addMouseListener(new MouseAdapter() {
    @Override
    public void mouseClicked(MouseEvent e) {
        int selectedRow = tblDSPC.getSelectedRow(); 
        if (selectedRow >= 0) {
            
            String maThoLamDan = modelDSPC.getValueAt(selectedRow, 2).toString();
            String tenThoLamDan = modelDSPC.getValueAt(selectedRow, 3).toString();
            String tenCongDoan = modelDSPC.getValueAt(selectedRow, 1).toString();

           
            txtMaThoLamDan.setText(maThoLamDan);
            txtTenThoLamDan.setText(tenThoLamDan);
            txtTenCongDoan.setText(tenCongDoan);
